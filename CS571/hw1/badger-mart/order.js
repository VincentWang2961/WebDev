...
function calculateSubtotal() {
	...
	for(const iid of ITEMS){
		const quantity = parseInt(document.getElementById(iid + "-quantity").value);
		const price = parseFloat(document.getElementById(iid + "-price").innerText); 
...
function calculateSalesTax() {
	...
	let tax = getSalesTaxRateForState(state) * calculateSubtotal();
	return roundMoney(tax); // TODO calculateSalesTax
}
...
document.getElementById("btn-checkout").addEventListener("click", () => {
	alert("Your total bill is: $" + (calculateSalesTax() + calculateSubtotal()).toFixed(2));
});